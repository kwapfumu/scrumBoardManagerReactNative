...
export default class ReleaseBacklog extends Component {
	constructor (props) {
		super(props);
		StatusBarIOS.setStyle('light-content');
		this.state = {
			tasksDataSource:ds.cloneWithRows(this.props.backlogTasks),
			loaded:false
		};		
	},
	renderTaskRow(task){
						...
						<View style = {styles.taskDetailsContainerWrap}>
							<View style = {styles.taskNameNdeveloperNameContainer}>
								...
								<View style={styles.developerNameContainer}>
									<Icon name={task.status? 'todo':'inProgress'} color='#fff' size={23} style={{ padding:5 }} />
									<Text style={styles.rowText}>{task.developer}</Text>
								</View>
							</View>
							{/*StoryPointsBadge*/}
							<View style = {styles.StoryPointsBadgeContainer}>
