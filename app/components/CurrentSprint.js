...
export default class CurrentSprint extends Component {
	constructor (props) {
		super(props);
		StatusBarIOS.setStyle('light-content');
		this.state = {
			tasksDataSource:ds.cloneWithRows(this.props.currentSprintTasks),
			loaded:false
		};		
	},
	renderTaskRow(task){
			...
			<TouchableHighlight onPress={() => {this.props.navigator.push({
				...
				<View style = {styles.tasksListContainer}>
					...
					<View style = {styles.devIconContainer}>
						<Image source={{uri:task.developer.image}} style={styles.devImage} resizeMode='contain' />
					</View>
					{/*taskDetails*/}
						...
						<View style = {styles.taskDetailsContainerWrap}>
							<View style = {styles.taskNameNdeveloperNameContainer}>
								...
								<View style={styles.developerNameContainer}>
									<Icon name={this.somefunction} color='#fff' size={23} style={{ padding:5 }} />
									<Text style={styles.rowText}>{task.taskDescription}</Text>
								</View>
							</View>
							{/*StoryPointsBadge*/}
							<View style = {styles.StoryPointsBadgeNStatusContainer}>
...
const styles = StyleSheet.create({
	...
	StoryPointsBadgeNStatusContainer:{
		flex: 1,
		alignItems: "center"
	},
	editIconContainer:{
